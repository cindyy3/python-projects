...
def get_pixel_at(pixel_grid, i, j):
    ...
    if i < 0 or j < 0:
        return 0
    elif j > len(pixel_grid[0]) - 1 or i > len(pixel_grid) - 1:
        return 0
    else:
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    pixel = get_pixel_at(pixel_grid, i, j)
    pixel_sum += pixel
    pixel = get_pixel_at(pixel_grid, i+1, j)
    pixel_sum += pixel
    pixel = get_pixel_at(pixel_grid, i, j-1)
    pixel_sum += pixel
    pixel = get_pixel_at(pixel_grid, i, j+1)
    pixel_sum += pixel
    pixel = get_pixel_at(pixel_grid, i-1, j)
    pixel_sum += pixel
    pixel = get_pixel_at(pixel_grid, i-1, j-1)
    pixel_sum += pixel
    pixel = get_pixel_at(pixel_grid, i+1, j+1)
    pixel_sum += pixel
    pixel = get_pixel_at(pixel_grid, i-1,j+1)
...
def blur(pixel_grid):
    ...
    for e in range(2*(len(pixel_grid) - 1)):
        if len(newrow) == len(pixel_grid[0]):
            blurred_grid.append(newrow)
            newrow = []
            if len(blurred_grid) == len(pixel_grid):
                break
        for f in range(len(pixel_grid[0])):
            number = average_of_surrounding(pixel_grid, e, f)
...
write_image(input_file, input_grid)
write_grid(input_file, input_grid)
